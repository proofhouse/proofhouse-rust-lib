...
`just lint-commit-msg` mirrors the commit-msg hook. Under the commit scope, vale reads the draft for AI commit tells via `ai-tells-commits`, and cspell checks it against the commit dictionary. The commitlint check covers Conventional Commits shape, and commit-trailers covers trailer order. Running it while drafting surfaces problems early, rather than at the commit-msg hook where a late failure interrupts the commit.
